...
namespace april_content
{
  ...
  float ConnectorOrderParameter::m_orderParameterTimingPower(0.f);
  ...
  pandora::StatusCode ConnectorCleaningTool::Process(const pandora::Algorithm& /*algorithm*/, const pandora::CaloHitList *const /* caloHitList */)
  {
    ...
    ConnectorOrderParameter::m_orderParameterTimingPower   = m_activatedTiming ? m_orderParameterTimingPower : 0.f;
  ...
  pandora::StatusCode ConnectorCleaningTool::CleanCaloHits(const pandora::CaloHitList *const pCaloHitList) const
  {
    ...
    for(auto caloHit : caloHitVector)
    {
      ...
      //Added by TP
      float timing = 1.f; //doesn't impact the order parameter if timing not activated

      if(m_activatedTiming)
      {
        if (pFromCaloHit->GetSmearedTime() != 0.f && pConnector->GetTo()->GetSmearedTime() != 0.f)
        {
          timing = std::fabs(pConnector->GetSmearedTiming());
        }
      }
    ...
    ConnectorOrderParameter orderParameter(distance, angle, timing, nConnectors, creationStage, hitPos);
  ...
  pandora::StatusCode ConnectorCleaningTool::ReadSettings(const pandora::TiXmlHandle xmlHandle)
  {
    ...
    m_orderParameterTimingPower = 1.f;
    PANDORA_RETURN_RESULT_IF_AND_IF(pandora::STATUS_CODE_SUCCESS, pandora::STATUS_CODE_NOT_FOUND, !=, pandora::XmlHelper::ReadValue(xmlHandle,
        "OrderParameterTimingPower", m_orderParameterTimingPower));

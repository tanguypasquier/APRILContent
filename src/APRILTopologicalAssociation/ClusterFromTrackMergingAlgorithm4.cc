...
namespace april_content
{
  ...
  void ClusterFromTrackMergingAlgorithm4::SearchProperClusters(const pandora::Track* pTrack, APRILCluster* startingCluster)
  {
#if __DEBUG__
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm4::CleanClusterForMerging(std::vector<APRILCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm4::GetEligibleClusters(std::vector<APRILCluster*>& clusterVector) const
  {
	clusterVector.clear();

/**
 *  @file   ConeBasedMergingAlgorithm.cc
 * 
...
StatusCode ConeBasedMergingAlgorithm::Run()
{
    ...
    for (ClusterVector::reverse_iterator iterI = daughterVector.rbegin(), iterIEnd = daughterVector.rend(); iterI != iterIEnd; ++iterI)
    {
		...
		pandora::ClusterVector::iterator parentVectorIter(std::find(parentVector.begin(), parentVector.end(), pDaughterCluster));
...
float ConeBasedMergingAlgorithm::GetFractionInCone(const pandora::Cluster *const pParentCluster, const pandora::Cluster *const pDaughterCluster,

...
namespace arbor_content
{
  ...
  void NeutralFragmentMergingAlgorithm::SearchProperClusters(const pandora::Track* pTrack, ArborCluster* startingCluster)
  {
#if __DEBUG__
  ...
  pandora::StatusCode NeutralFragmentMergingAlgorithm::CleanClusterForMerging(std::vector<ArborCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode NeutralFragmentMergingAlgorithm::GetEligibleClusters(std::vector<ArborCluster*>& clusterVector) const
  {
	clusterVector.clear();

...
namespace arbor_content
{
  ...
  void ClusterFromTrackMergingAlgorithm2::SearchProperClusters(const pandora::Track* pTrack, ArborCluster* startingCluster, 
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm2::CleanClusterForMerging(std::vector<ArborCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm2::GetEligibleClusters(std::vector<ArborCluster*>& clusterVector) const
  {
	clusterVector.clear();

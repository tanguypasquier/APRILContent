...
#include "ArborTopologicalAssociation/ClusterFromTrackMergingAlgorithm5.h"
...
namespace arbor_content
{
  ...
  void ClusterFromTrackMergingAlgorithm5::SearchProperClusters(ArborCluster* startingCluster, 
  ...
  void ClusterFromTrackMergingAlgorithm5::SearchProperClusters(const pandora::Track* pTrack, ArborCluster* startingCluster, 
		  std::vector<arbor_content::ArborCluster*>& properClusters)
  {
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm5::CleanClusterForMerging(std::vector<ArborCluster*>& clusterVector)
  {
	for(int i = 0; i < clusterVector.size(); ++i)
	{
		auto& cluster = clusterVector.at(i);
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm5::GetEligibleClusters(std::vector<ArborCluster*>& clusterVector) const
  {
	clusterVector.clear();
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm5::FindBestParentCluster(const pandora::Cluster *const pDaughterCluster, const pandora::ClusterVector &clusterVector,
  ...
  pandora::StatusCode ClusterFromTrackMergingAlgorithm5::GetClusterBackwardDirection(const pandora::Cluster *const pCluster, pandora::CartesianVector &backwardDirection, pandora::CartesianVector &innerPosition) const
  {
    pandora::CartesianVector centroid(0.f, 0.f, 0.f);
    PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS, !=, ClusterHelper::GetCentroid(pCluster, centroid));

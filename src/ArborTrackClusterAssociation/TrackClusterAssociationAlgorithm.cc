...
#include "ArborObjects/ArborTrack.h"
...
pandora::StatusCode TrackClusterAssociationAlgorithm::Run()
{
    ...
    for (const pandora::Track *const pTrack : trackVector)
    {
        ...
        ArborTrackStates trackStates;
        const ArborTrack* arborTrack = dynamic_cast<const ArborTrack*>(pTrack);
        if( arborTrack )
        {
            trackStates = arborTrack->GetTrackStates();
        }
        else
        {
        ...
        for (auto const& trackState : trackStates)
        {
            ...
            for (const pandora::Cluster *const pCluster : nearbyClusterList)
            {
                ...
                if (pandora::STATUS_CODE_SUCCESS != arbor_content::ClusterHelper::GetTrackClusterDistance(&trackState, pCluster, m_maxSearchLayer, m_parallelDistanceCut,
